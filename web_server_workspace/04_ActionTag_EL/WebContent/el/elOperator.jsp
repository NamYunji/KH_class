...
<%
	...
	List<String> list = null; // 선언만, null인 상태
	list = new ArrayList<>(); // 객체 할당, null은 아니고 요소가 없는 상태
...
<body>
	...
	<!-- 스크립틀릿의 동등비교연산 - 자바의 동등비교연산 - 주소값 비교 -> false
		 el의 동등비교연산 - 내부적으로 equals 사용 -->
	<p><%= str1 != str2 %> ${str1 != str2} ${str1 ne str2 }</p> <!-- true false false -->
	
	<p>${p1 == p2} ${p1 eq p2}</p> <!-- [오버라이드 전] false false -> [오버라이드 후] true true -->
	<!-- 필드의 내용이 같아도 다른 객체이므로 true -->
	<!-- 필드의 내용이 같으면 같은 객체로 취급하고 싶다면? 
		 equals와 hash코드 오버라이드 -->
	
	<!-- 객체가 null이거나 요소가 없는지의 여부 -->
	<p>${empty list} ${not empty list}</p>	
	<!-- list --- [변수만 선언시, null]true -> [객체 할당만, 요소가 없음]true -> [요소추가]false -->
