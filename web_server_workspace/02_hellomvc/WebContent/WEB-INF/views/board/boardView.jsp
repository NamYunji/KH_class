	...
	<div class="container">
		...
		<div class="comment-table1">
			<form
				...
				<input type="submit" id="comment-enroll-btn" value="등록" />
		</div>
				<!--table#tbl-comment -->
		<% if(commentList != null && !commentList.isEmpty()){ %>	
		<table id="tbl-comment">
...
<%=clickLike%>
	==
<%=teamMemberboard.getTeamAId()%>
	) {
							$('label')
									.css(
											{
												"background-image" : "url(../image/커뮤니티/like2.png)"
											});
						} else if (
<%=clickLike%>
	!= null
...
<%=clickLike%>
	== 0) {
							$('label')
									.css(
											{
												"background-image" : "url(../image/커뮤니티/like1.png)"
											});
						}
					});
	$(document)
...
<%=teamMemberboard.getTeamAId()%>
												...
												if ($("#like1").is(":checked")) {
													$(
															document.teamMemberboardLikeFrm)
															.submit();
													$('label')
															.css(
																	{
												...
												if ($("#like1").is(":checked")) {
													$(
															document.memberboardLikeFrm)
															.submit();
												} else {
													/* $("#like-cnt").html(likeCnt); */
													$(
															document.memberboardLikeCancelFrm)
															.submit();
												}
											}
										});
