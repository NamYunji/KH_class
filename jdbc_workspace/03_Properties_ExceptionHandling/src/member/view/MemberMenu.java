...
public class MemberMenu {
	...
	public void mainMenu() {
		...
		do {
			...
			//사용자 입력값을 choice 변수에 담아옴
			//숫자가 아닌 것을 입력하면 "잘못 입력하셨습니다"가 아닌 오류 (input mismatch exception)
			//int타입에 nextInt()를 받는 것 대신, String타입에 next()로 처리
			//문자열이라도 어차피 안에서 equals 비교연산이 일어나기 때문에 가능!
			//cf. case문의 literal로 가능한 것 : case (리터럴 정수 or 문자 or 문자열 )
			...
			case "0" : 
				...
				if (sc.next().charAt(0) == 'y') {
					return;
					//호출한 곳인 run으로 리턴 -> 다음 코드인 출력문 '프로그램 종료' 실행 
					//-> 다음 코드 없음 - terminated
				}
				break;
			default :
